...
def get_stations_for_pnr(request):
	...
	if request.method == 'POST':
		try:
			...
			qry = "with trainno as (select train_no_id as trainno from pnr where pnr_no = %s) "
			qry += "with train_stops as (select station_id, stop_num from stop, trainno where train_no_id = trainno.trainno) "
